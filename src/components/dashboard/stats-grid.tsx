    ...
    return (
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="bg-zinc-900/80 border border-zinc-800 rounded-xl p-5 animate-pulse h-28" />
        ))}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <StatCard
        ...
        subtitle={`${stats.totalAirdrops} total tracked`}
        icon="🎯"
        trend="up"
      />
      <StatCard
      ...
      <StatCard
        ...
        subtitle={`${stats.totalWallets} total wallets`}
        icon="👛"
      />
      <StatCard
        title="Running Tasks"
        ...
        icon="🏃"
        trend={stats.runningTasks > 0 ? "up" : "neutral"}
      />
      <StatCard
